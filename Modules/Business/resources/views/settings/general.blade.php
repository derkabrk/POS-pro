                    ...
                    <div class="order-form-section">
                        <form action="{{ route('business.settings.update', $setting->id ?? 0) }}" method="post" enctype="multipart/form-data" class="ajaxform_instant_reload">
                            ...
                            <div class="add-suplier-modal-wrapper d-block">
                                <div class="row g-4">
                                    <div class="col-lg-6">
                                        <label class="form-label fw-semibold text-secondary">{{ __('Business Category') }}</label>
                                    ...
                                    <div class="col-lg-6">
                                        <label class="form-label fw-semibold text-secondary">{{ __('Company And Business Name') }}</label>
                                    ...
                                    <div class="col-lg-6 mt-2">
                                        <label class="form-label fw-semibold text-secondary">{{ __('Phone Number') }}</label>
                                    ...
                                    <div class="col-lg-6 mt-2">
                                        <label class="form-label fw-semibold text-secondary">{{ __('Address') }}</label>
                                        <input type="text" name="address" value="{{ $business->address }}" class="form-control rounded-3 shadow-sm" placeholder="{{ __('Enter Address') }}">
                                    </div>
                                    <div class="col-lg-6 mt-2">
                                        <label class="form-label fw-semibold text-secondary">{{ __('VAT/GST Title') }}</label>
                                    ...
                                    <div class="col-lg-6 mt-2">
                                        <label class="form-label fw-semibold text-secondary">{{ __('VAT/GST Number') }}</label>
                                    ...
                                    <div class="col-lg-6 settings-image-upload">
                                        <label class="form-label fw-semibold text-secondary mb-2">{{ __('Invoice Logo') }}</label>
                                        <div class="upload-img-v2 d-flex align-items-center gap-3 p-3 bg-light rounded-3 shadow-sm border border-1">
                                            <label class="upload-v4 settings-upload-v4 m-0 position-relative" style="cursor:pointer;">
                                                ...
                                                <span class="position-absolute bottom-0 end-0 bg-primary text-white rounded-circle p-1" style="font-size: 1.2rem; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
                                            ...
                                            <div class="flex-grow-1">
                                                ...
                                                <div class="d-flex gap-2">
                                                    <button type="button" class="btn btn-outline-primary btn-sm rounded-pill px-3" onclick="document.querySelector('input[name=invoice_logo]').click();"><i class="ri-upload-cloud-2-line me-1"></i>{{ __('Change') }}</button>
                                                    <button type="button" class="btn btn-outline-danger btn-sm rounded-pill px-3" onclick="document.getElementById('invoice_logo').src='{{ asset('assets/images/icons/upload-icon.svg') }}'; document.querySelector('input[name=invoice_logo]').value='';"><i class="ri-delete-bin-6-line me-1"></i>{{ __('Remove') }}</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="text-center mt-5">
                                            <button type="submit" class="btn btn-primary btn-lg rounded-pill px-5 shadow submit-btn"><i class="ri-save-3-line me-2"></i>{{ __('Update') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
