    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 shadow-lg border-0">
            <div class="modal-header bg-primary text-white rounded-top-4">
                <h4 class="modal-title fw-bold mb-0">{{ __('Create Payment Type') }}</h4>
                ...
                <form action="{{ route('business.payment-types.store') }}" method="post" enctype="multipart/form-data" class="ajaxform_instant_reload">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="name" required class="form-control form-control-lg" placeholder="{{ __('Enter Name') }}">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">{{ __('Status') }}</label>
                            <select name="status" required class="form-select form-select-lg">
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        <button type="reset" class="btn btn-outline-secondary rounded-pill px-4">{{ __('Reset') }}</button>
