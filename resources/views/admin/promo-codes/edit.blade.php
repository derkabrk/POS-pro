...
@section('content')
<div class="card">
    <div class="card-header"><h4>{{ __('Edit Promo Code') }}</h4></div>
    <div class="card-body">
        <form action="{{ route('admin.promo-codes.update', $promoCode) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="active" id="active" class="form-control">
                    <option value="1" {{ old('active', $promoCode->active) == 1 ? 'selected' : '' }}>{{ __('Active') }}</option>
                    <option value="0" {{ old('active', $promoCode->active) == 0 ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                </select>
            </div>
